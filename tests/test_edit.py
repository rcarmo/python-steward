...
@pytest.mark.parametrize("initial,old_str,new_str,expected_output,expected_content", [
    ("hello world\n", "world", "universe", "Replaced", "hello universe\n"),
    ("def foo():\n    pass\n", "def foo():\n    pass", "def foo():\n    return 42", "Replaced", "def foo():\n    return 42\n"),
    ("hello world\n", " world", "", "Deleted", "hello\n"),
])
def test_edit_success(tool_handlers, sandbox: Path, test_file, initial, old_str, new_str, expected_output, expected_content):
    f = test_file(initial)
...
@pytest.mark.parametrize("content,old_str,error_match", [
    ("hello", "xyz", "not found"),
